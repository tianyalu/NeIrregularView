...
public class IrregularMenuView extends View {
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        path.lineTo((float)(cx + outerCr * Math.sin(sweepAngleOuter / 2 * Math.PI / 180)),
                (float) (cy - outerCr * Math.cos(sweepAngleOuter * Math.PI / 360)));
        path.addArc(outerRectF, -45 - disAngleOuter, -sweepAngleOuter);
        path.lineTo((float)(cx - innerCr * Math.sin(sweepAngleInner * Math.PI / 360)),
                (float) (cy - innerCr * Math.cos(sweepAngleInner * Math.PI / 360)));
